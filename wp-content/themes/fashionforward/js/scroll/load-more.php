...
function aq_load_more($page = 2, $archive = null) {
	...
	if( $maxpage >= 2) {
		$output .= '<div id="load-more">';
			...
			$output .= '<a href="#" data-pagenum="'. $page .'" data-maxpage="'. $maxpage .'" data-archive_type="'. $archive_type .'" data-archive_id="'. $archive_id .'" data-post_format="'. $post_format .'" data-author="'. $author .'" data-s="'.get_query_var('s').'">'. __('load more', 'framework') .'</a>';
		$output .= '</div>';
	}
